...
fetched_from = 'culturedesigner.zohorecruit.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://culturedesigner.zohorecruit.com/jobs/Careers' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://culturedesigner.zohorecruit.com/jobs/Careers/{0}".format(x.get("url", "")) for x in sl.M]
    ##############################
    #Para pruebas
    # sl.printWR()
    #sl.printM()
    ...
    for url in sl.WR:
        ...
        for offer in sl.M:
            ...
            ad['salary'] = offer.get("salary",'0').decode("unicode-escape").encode("utf-8")
