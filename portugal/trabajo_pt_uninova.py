...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://www.uninova.pt/jobs-opportunities/active?page={page}' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://www.uninova.pt/job-opportunity{0}".format(x.get('url', '')) for x in sl.M]
