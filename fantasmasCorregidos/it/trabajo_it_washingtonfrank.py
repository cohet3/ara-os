...
fetched_from = 'washingtonfrank.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://www.washingtonfrank.com/erp-jobs?newJobs=&keyword=&location=Italy&jobType=both&page={page}&remote=&security=&salaryFrom=&salaryTo=&salaryCurrency=&segment=&product=',
        )
    }
     ),
)
...
def crawl(web): 
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://www.washingtonfrank.com{0}".format(x.get("url", "")) for x in sl.M]
