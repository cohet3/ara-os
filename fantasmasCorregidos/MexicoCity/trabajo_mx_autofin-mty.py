...
fetched_from = 'autofin-mty.sherlockhr.computrabajo.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://autofin-mty.sherlockhr.computrabajo.com/ofertas/' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    headers = { 
        ...
        'pragma':'no-cache',
        'referer':'https://autofin-mty.sherlockhr.computrabajo.com/ofertas/',
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://autofin-mty.sherlockhr.computrabajo.com{0}".format(x.get('url', '')) for x in sl.M]
