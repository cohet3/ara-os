...
fetched_from = 'grupo-bovinos.sherlockhr.computrabajo.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://grupo-bovinos.sherlockhr.computrabajo.com/ofertas/' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    headers = { 
        ...
        'pragma':'no-cache',
        'referer':'https://grupo-bovinos.sherlockhr.computrabajo.com/ofertas/',
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://grupo-bovinos.sherlockhr.computrabajo.com{0}".format(x.get('url', '')) for x in sl.M]
