...
fetched_from = 'bmwgroup.referrals.selectminds.com'
...
xtr_url = '''url
<a href=\\\\"https://bmwgroup.referrals.selectminds.com/jobs/
\\\\"'''
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://bmwgroup.referrals.selectminds.com/',
        )
    }
     ),
)
...
#################POST################# realiza búsqueda y saca jobSearchID
    headers= {
        ...
        'Cookie':cookies,
        'Host':'bmwgroup.referrals.selectminds.com',
        'Origin':'https://bmwgroup.referrals.selectminds.com',
        'Referer':'https://bmwgroup.referrals.selectminds.com/',
    ...
    data = '''keywords=&geo_location=M%C3%A9xico&geo_lat=19.43196&geo_long=-99.13316&geo_level=country&geo_area_id=22000493&geo_region_area_id=&geo_country_area_id=22000493&geo_search_radius_units=km&geo_search_radius_km=50'''
    
    req = requests.post(url = 'https://bmwgroup.referrals.selectminds.com/ajax/jobs/search/create', headers = headers, data = data, verify = False)
    datos = req.text
    #print datos
    #exit(0)
    ...
    #################POST################# Obtiene los resuldados de la búsqueda anterior para sacar las urls
        ...
        data ='''JobSearch.id=''' + searchID + '''&page_index=''' + str(page) + '''&site-name=default763&include_site=true'''
        urlResults = 'https://bmwgroup.referrals.selectminds.com/ajax/content/job_results?JobSearch.id={0}&page_index={1}&site-name=default763&include_site=true'.format(searchID, page)
        req = requests.post(url = urlResults, headers = headers, data = data, verify = False)
        datos = req.text
        # print datos
        # exit(0)
        ...
        sl.extract(xtr_url)
        sl.WR = ["https://bmwgroup.referrals.selectminds.com/jobs/{0}".format(x.get("url", "")) for x in sl.M]
