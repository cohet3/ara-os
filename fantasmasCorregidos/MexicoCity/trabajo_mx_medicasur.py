...
fetched_from = 'medicasur.sherlockhr.computrabajo.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://medicasur.sherlockhr.computrabajo.com/ofertas/' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    headers = { 
        ...
        'pragma':'no-cache',
        'referer':'https://medicasur.sherlockhr.computrabajo.com/ofertas/',
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://medicasur.sherlockhr.computrabajo.com{0}".format(x.get('url', '')) for x in sl.M]
