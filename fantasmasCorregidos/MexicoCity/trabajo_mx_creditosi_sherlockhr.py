...
fetched_from = 'creditosi.sherlockhr.computrabajo.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://creditosi.sherlockhr.computrabajo.com/ofertas/' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    headers = { 
        ...
        'pragma':'no-cache',
        'referer':'https://creditosi.sherlockhr.computrabajo.com/ofertas/',
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://creditosi.sherlockhr.computrabajo.com{0}".format(x.get('url', '')) for x in sl.M]
