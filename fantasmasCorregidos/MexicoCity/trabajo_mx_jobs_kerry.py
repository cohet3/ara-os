...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://jobs.kerry.com/search/?createNewAlert=false&q=&locationsearch=mexico&optionsFacetsDD_department=&optionsFacetsDD_country=&optionsFacetsDD_customfield1=&optionsFacetsDD_location=' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://jobs.kerry.com/job/{0}".format(x.get("url", "")) for x in sl.M]
    #Para pruebas
    #sl.printWR()
    #sl.printM()
    #sl.printstatus()
