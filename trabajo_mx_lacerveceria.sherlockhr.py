...
fetched_from = 'lacerveceria.sherlockhr.computrabajo.com'
...
pages = (
    ('Trabajo', {
        'trabajo_subcat': (
            'https://lacerveceria.sherlockhr.computrabajo.com/ofertas/' ,
        )
      }
    ),
)
...
def crawl(web):
    ...
    headers = { 
        ...
        'pragma':'no-cache',
        'referer':'https://sonoraforming.sherlockhr.computrabajo.com/ofertas/',
    ...
    sl.extract(xtr_url)
    sl.WR = ["https://lacerveceria.sherlockhr.computrabajo.com{0}".format(x.get('url', '')) for x in sl.M]
